  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ fontSize: 22, fontWeight: 700, color: '#030213', marginBottom: 4 }}>{event.title}</h2>
        <p style={{ fontSize: 13, color: '#6b7280', marginBottom: 16 }}>
          by {event.organizer} &middot; {formatDate(event.eventDate)}
        </p>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
          <StatusBadge status={getEventStatus(event)} />
          <span style={{ fontSize: 12, padding: '4px 10px', borderRadius: 20, background: '#f3f4f6', color: '#374151' }}>
            {getCategory(event)}
          </span>
          {event.location && (
            <span style={{ fontSize: 12, padding: '4px 10px', borderRadius: 20, background: '#f3f4f6', color: '#374151' }}>
        ...
        <p style={{ fontSize: 14, lineHeight: 1.6, color: '#374151', whiteSpace: 'pre-wrap' }}>
          {event.description}
        </p>
...
export default function AdminEventsModeration() {
  ...
  // Fetch all events (no approved filter)
  useEffect(() => {
    const fetchAllEvents = async () => {
      // Don't fetch if not an admin
  ...
  const filteredEvents = useMemo(() => {
    let result = events;
    if (activeTab !== 'All') {
      result = result.filter(e => getEventStatus(e) === activeTab);
    }
    if (searchQuery.trim()) {
  ...
  const handleApprove = async (eventId: string) => {
    ...
    try {
      ...
      setEvents(prev =>
        prev.map(e => (e.$id === eventId ? { ...e, approved: true, status: e.status === 'Rejected' ? 'Upcoming' : e.status } : e))
      );
  ...
  const handleReject = async (eventId: string) => {
    ...
    try {
      ...
      setEvents(prev =>
        prev.map(e => (e.$id === eventId ? { ...e, approved: false, status: 'Rejected' } : e))
      );
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: '#6b7280', margin: 0 }}>Review and manage submitted events</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {stats.pending > 0 && (
          ...
          {statCards.map(card => (
            ...
            >
              ...
              <div>
                ...
                <p style={{ fontSize: 24, fontWeight: 700, color: '#030213', margin: 0 }}>{card.value}</p>
              </div>
            </div>
          ))}
        </div>
            ...
            >
              ...
              {tab.count !== undefined && tab.count > 0 && (
                <span style={{
                  ...
                  fontSize: 11,
                  fontWeight: 700,
                  background: activeTab === tab.key ? 'rgba(255,255,255,0.2)' : '#fef3c6',
                  color: activeTab === tab.key ? '#fff' : '#bb4d00',
                }}>
                  {tab.count}
                </span>
              )}
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                          ...
                          <div>
                            <p style={{ fontSize: 14, fontWeight: 600, color: '#030213', margin: 0, lineHeight: 1.3 }}>
                              {event.title}
                            </p>
                      ...
                      <td style={{ padding: '12px 16px', fontSize: 13, color: '#374151' }}>
                        {event.submittedBy || event.createdByUserId || 'Unknown'}
                      </td>
                      {/* College */}
                      <td style={{ padding: '12px 16px', fontSize: 13, color: '#374151' }}>
                        {event.location || '—'}
