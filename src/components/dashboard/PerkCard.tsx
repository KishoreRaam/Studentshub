...
export default function PerkCard({ perk, onViewDetails, onToggleSave }: PerkCardProps) {
  const logoUrl = getLogoUrl(perk.title, (perk as any).website || (perk as any).claimLink);
  ...
  const handleViewDetails = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (btn) {
      ...
      ripple.style.cssText = `width:60px;height:60px;left:${e.clientX - rect.left - 30}px;top:${e.clientY - rect.top - 30}px;background:radial-gradient(circle,rgba(26,86,219,0.4) 0%,transparent 70%);`;
  ...
  return (
    <div className="nm-flat-hover rounded-2xl p-6 relative">
      ...
      >
        <Bookmark
          className={`w-4 h-4 ${perk.isSaved ? 'fill-[#1A56DB] text-[#1A56DB]' : 'text-gray-500 dark:text-gray-400'}`}
        />
      </button>
      ...
      {/* Content */}
      <div className="mb-5">
        <h3 className="text-base font-semibold text-gray-900 dark:text-white pr-8 mb-2">{perk.title}</h3>
        ...
        {perk.validity && (
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">{perk.validity}</p>
        )}
        <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{perk.description}</p>
      </div>
      ...
      <button
        ...
        className="nm-btn relative overflow-hidden w-full py-3 rounded-xl text-sm font-semibold text-[#1A56DB] dark:text-blue-400 flex items-center justify-center gap-2 cursor-pointer"
      >
