...
// Example perk data for demonstration
const examplePerks = [
  {
    id: 'jetbrains',
    name: 'JetBrains',
...
export function GitHubStudentPackLayout() {
      ...
      >
        <Card className="h-full border-0 shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white relative">
          ...
          <CardContent className="p-6 md:p-8 relative z-10 pt-16">
            ...
            {/* Header Section */}
            <header className="flex flex-col lg:flex-row lg:items-start lg:space-x-6 space-y-4 lg:space-y-0 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3 mb-6">
              {examplePerks.slice(0, 6).map((perk) => {
                const IconComponent = perk.icon;
                return (
                  <div key={perk.id} className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center hover:bg-white/15 transition-colors">
                    <IconComponent className="w-5 h-5 mx-auto mb-2 text-blue-300" />
                    <div className="text-xs text-gray-300 truncate">{perk.name}</div>
                  </div>
                );
              })}
            </div>
            ...
            {/* Call to Action */}
            <footer className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
            ...
            <DialogContent className="max-w-7xl h-[90vh] overflow-hidden p-0 bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white border-gray-700">
              ...
              <div className="flex flex-col h-full max-h-[90vh]">
                ...
                <DialogHeader className="p-4 md:p-6 border-b border-gray-700 flex-shrink-0">
                  ...
                  {/* Header Content */}
                  <header className="flex flex-col lg:flex-row lg:items-start lg:space-x-6 space-y-4 lg:space-y-0">
                            ...
                            >
                              <Card className="h-full bg-gray-800/50 border border-gray-700/50 hover:border-gray-600 transition-all duration-200 overflow-hidden">
                                <CardContent className="p-4 h-full flex flex-col">
                                  ...
                                  {/* Perk Icon */}
                                  <div className={`w-12 h-12 rounded-lg ${perk.bgColor} p-3 mb-3 flex items-center justify-center flex-shrink-0`}>
                                    <IconComponent className="w-6 h-6 text-white" />
                                  </div>
                                  
                                  {/* Perk Content */}
                                  <div className="flex-1 flex flex-col">
                                    <h4 className="text-white mb-1 text-sm md:text-base">{perk.name}</h4>
                                    <p className="text-blue-300 text-xs md:text-sm mb-2 font-medium">{perk.offer}</p>
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-700/30">
                                      ...
                                      <span className="text-green-400 text-xs md:text-sm font-medium">{perk.value}</span>
                                    </div>
                                  </div>
                ...
                {/* FIXED FOOTER */}
                <footer className="p-4 md:p-6 border-t border-gray-700 flex justify-end flex-shrink-0">
                  <Button 
                    variant="outline"
