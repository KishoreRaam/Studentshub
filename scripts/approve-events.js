const sdk = require('node-appwrite');

const DATABASE_ID = '68d3d183000b0146b221';
const COLLECTION_ID = 'events';

const apiKey = 'standard_';

async function main() {
    const client = new sdk.Client()
        .setEndpoint(process.env.APPWRITE_ENDPOINT || 'https://fra.cloud.appwrite.io/v1')
        .setProject(process.env.APPWRITE_PROJECT || '68d29c8100366fc856a6')
        .setKey(apiKey);

    const databases = new sdk.Databases(client);

    try {
        const docs = await databases.listDocuments(DATABASE_ID, COLLECTION_ID);
        for (const d of docs.documents) {
            if (!d.category) {
                console.log(`Deleting invalid event ${d.title} (missing category)`);
                await databases.deleteDocument(DATABASE_ID, COLLECTION_ID, d.$id);
            } else {
                await databases.updateDocument(DATABASE_ID, COLLECTION_ID, d.$id, {
                    approved: true,
                    status: 'Upcoming'
                });
                console.log(`Updated event ${d.title} to approved: true`);
            }
        }
    } catch (e) {
        console.error('Update docs error:', e);
    }
}

main();
